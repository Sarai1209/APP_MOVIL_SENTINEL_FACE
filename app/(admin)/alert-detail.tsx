...
export default function AlertDetailScreen() {
  ...
  const handleResolve = () => {
    Alert.alert(
      'Resolver alerta',
      '¿Confirmas que esta alerta ha sido atendida y puede marcarse como resuelta?',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setResolving(true);
            try {
              await api.resolveAlert(Number(id), user?.id ?? 1);
              setAlert((prev: any) => ({ ...prev, resolved: true, resolved_by: user?.name }));
            } catch {
  ...
  if (loading) {
    return (
      <LinearGradient colors={['#050514', '#0D0D2B', '#050514']} style={styles.center}>
        <ActivityIndicator color={C.pinkNeon} size="large" />
      </LinearGradient>
    );
  }
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoRow}>
            ...
            <Text style={[styles.infoValue, {
              color: alert.resolved ? Colors.Status.success : Colors.Status.warning,
            }]}>
          ...
          {alert.resolved_by_name && (
            <View style={[styles.infoRow, styles.rowDivider]}>
              ...
              <Text style={styles.infoValue}>{alert.resolved_by_name}</Text>
            </View>
          )}
          {alert.log_id && (
            <View style={[styles.infoRow, styles.rowDivider]}>
...
const styles = StyleSheet.create({
  ...
  resolveBtn: {
    ...
    backgroundColor: Colors.Status.success, borderRadius: 14, paddingVertical: 15, marginBottom: 12,
  },
  resolveTxt: { color: '#050514', fontSize: 16, fontWeight: '700' },
  resolvedBanner: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    backgroundColor: 'rgba(0,229,160,0.1)', borderRadius: 12,
    borderWidth: 1, borderColor: 'rgba(0,229,160,0.25)', paddingVertical: 14,
  },
