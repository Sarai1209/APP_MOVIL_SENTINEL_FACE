    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.summaryRow}>
        {[
          ...
          <LinearGradient key={i} colors={[`${s.color}20`, `${s.color}05`]} style={[styles.summaryCard, { borderColor: `${s.color}30` }]}>
      ...
      {Object.entries(grouped).map(([date, items]) => (
        <View key={date}>
          <Text style={styles.dateLabel}>{date} · {items.length} evento{items.length !== 1 ? 's' : ''}</Text>
          {items.map(item => {
            ...
            return (
              <View key={item.log_id} style={styles.reportRow}>
                <View style={[styles.iconWrap, { backgroundColor: `${color}15` }]}>
                  {ok ? <CheckCircle size={16} color={color} /> : spoof ? <ShieldAlert size={16} color={color} /> : <XCircle size={16} color={color} />}
                </View>
                <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  dateLabel: { color: C.textMuted, fontSize: 11, fontWeight: '600', letterSpacing: 1.5, marginBottom: 8, marginTop: 16 },
  reportRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: C.border },
